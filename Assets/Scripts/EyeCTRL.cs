...
public class EyeCTRL : MonoBehaviour
{
    ...
    private float xLimit, yLimit, listeningLimit, talkingLimit, saccadeOffset;
    public bool followFace, followMouse, saccade;
    ...
    // (X, Y, Z) --> (xFactor, yFactor, offset)
    // Where:
    // - X and Y are the values that multiply
    // the components of user/mouse movement,
    // in order to convert image coordinates
    // to world (Unity) coordinates;
    // - Offset: the amount that is added to
    // the Y component of eyes/head in order
    // to fix ilumination and gaze alignment
    //problems.
    public Vector3 faceComponents, mouseComponents; 
    ...
    void Update()
    {
        if (!mc.isSleeping)
        {
            ...
            if (!followFace && !followMouse && !saccade)
            {
                if (saccadeCoroutine != null)
                {
                    StopCoroutine(saccadeCoroutine);
    ...
    {
        Ray face = Camera.main.ScreenPointToRay(followPoint);

        if (initX == 0) initX = face.origin.x;
        if (initY == 0) initY = face.origin.y;

        Vector3 position = new Vector3(0f, 0f, 0f);
        position.x = (face.origin.x - initX) * faceComponents.x;
        position.y = (face.origin.y - initY) * faceComponents.y + faceComponents.z;

        transform.position = position;
        headCTRL.FollowMouse(position.x, position.y);
    }
    ...
    void FollowMouse()
    {
        ...
        position.x = mouse.origin.x * mouseComponents.x;
        position.y = mouse.origin.y * mouseComponents.y + mouseComponents.z;

        // Applies transformation into ECA's iris
        if (!saccade || true) transform.position = position;
    ...
    private IEnumerator Saccade()
    {
        ...
        while (true)
        {
            ...
            if (agentMode == "listening")
            {
                if (gazeMode == "mutual") saccadeFrames = UnityEngine.Random.Range(190.4f, 284.6f);
